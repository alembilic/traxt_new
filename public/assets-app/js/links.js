function storeBacklinks() {
    ...
    if (!links.length) {
        swal({type: 'success', title: 'Import Failed', text: 'Links not selected'});
    }

    $.ajax({
        url: '<?=_URL_API_?>domain/' + domain_id + '/importBackLinks',
        data: JSON.stringify({'links': links}),
        headers: {'X-Auth-Token': apiToken, 'Content-Type': 'application/json'},
        type: 'POST',
        success: function () {
            swal({type: 'success', title: 'Import Complete', text: ''}, function () {
                location.reload()
            })
        },
        error: function () {
            swal({type: 'warning', title: 'Something went wrong', text: 'Please, try again later.'}, function () {
                location.reload()
            });
        }
    });
...
function loadBackLinks(domain_id) {
    $.ajax({
        url: '/api/domain/' + domain_id + '/retrieveBackLinks',
        ...
        error: function () {
            swal({type: 'warning', title: 'Something went wrong', text: 'Please, try again later.'});
        }
    });
}
...
$(function () {
    ...
    $('.import-btn').click(function () {
        $('.ib-domains-list').show();
        return false;
    });
    $('.ib-domain').click(function () {
        $('.ib-domains-list').hide();
        selectDomainPopup($(this).html(), $(this).data('id'));
    ...
    $('body').on("keyup", "#search-url", function () {
        var search = this.value.toLowerCase().trim();
        if (!search) {
            $('.popup-table tbody tr').show();
        } else {
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="extended-input">
                            ...
                            <textarea class="form-control" placeholder="Autosize height based on content lines"></textarea>
                            <p class="text-start">Enter one URL per line</p>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-primary">Import URL’s</button>
                    </div>
                </div>
            `;
        Swal.fire({
        ...
        form.innerHTML = ` <div class="Found-modal">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <input class="form-control" type="search" placeholder="search link">
                                ...
                                <tr>
                                    <th>
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" value="" id="header-1">
                                        </div>
                                    </th>
